...
# 개요
-  이 문서는 **제2기(이후) 기술/연구 테크트리**를 ‘시대별 게이트 + 선후관계(엣지)’로 관리하기 위한 **전역 템플릿**이다.
...
-  적용 기수: **제2기 이후**
-  운영 방식(권장):
  -  **시대 문서(인덱스)**: 시대별 게이트 목록/요약 표
  -  **게이트 문서(노드)**: 각 기술/제도/연구 게이트의 단일 진실(Single Source of Truth)
  -  **엣지 레지스트리(관계)**: `A → B` 선후관계만 모은 전역 표(트리 생성/검증용)
...
> [!tip] 운영 원칙
> 시대 문서는 **얇게(표 중심)**, 게이트 문서는 **두껍게(설명/근거/관계 중심)**, 엣지는 **관계만**.
...
> 오버레이는 “표기 선택 사항”이다. 쓰려면 **전역 규칙(이 문서)**에 정의하고, 안 쓰면 과감히 빼자.
...
## 새 게이트 추가 시
- [ ] `gate_id`가 **유일**한가(중복 없음)
- [ ] 시대 인덱스 표에 `gate_id + 인링크`가 들어갔는가
- [ ] 게이트 문서(frontmatter)의 `tree_map`이 **3단계 분류체계**에 실제로 존재하는가
- [ ] `prereq/unlocks`가 있으면 엣지 레지스트리에도 동일하게 반영했는가
- [ ] “왜 게이트인가”가 **시대 변곡점**과 연결되어 있는가(서사/정치/경제 중 최소 1)

## 선후관계(엣지) 점검
- [ ] 의도치 않은 **사이클**이 생기지 않는가(필요하면 `alt_path`로 분리)
- [ ] 지나치게 허브 1개에 몰리지 않는가(병목이 의도인지 확인)
- [ ] 시대를 넘어가는 엣지(예: `2-01 → 2-04`)는 **설명(note)**를 붙였는가
